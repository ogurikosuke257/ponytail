...
name: ponytail-debt
description: >
  コードベース中のすべての `ponytail:` コメントを負債台帳へ集める。Ponytailが残した意図的な簡略化と先送りを、後でやるつもりのまま腐らせない。「ponytailの負債」「/ponytail-debt」「何を先送りしたか」「ショートカット一覧」「ponytail台帳」または「後でやる印を一覧にして」で使う。1回限りの報告で、変更はしない。
---
